...
import today.Miscible.gui.clickgui.options.UIMode;
import today.Miscible.gui.clickgui.options.UISlider;
import today.Miscible.gui.clickgui.options.UIToggleButton;
            ...
            if (m.hasValues()) {
                ...
                if (arrowHover) {
                    if(m.openValues) {
                		...
                		RenderUtil.drawRect(this.x + this.width - 2, yAxis + (height - size) / 2 - 5, this.x + this.width, yAxis + (height - size) / 2 + 1 + 10, new Color(color.getRed(), color.getGreen(), color.getBlue(), 200).getRGB());
                	}
                } else {
                	if(m.openValues) {
