...
public class Criticals extends Mod {
	public Value mode = new Value("Criticals", "Mode", new String[]{"Vanilla" , "Hypixel" , "Mineplex"},  0);
	public Value delay = new Value("Criticals","Delay", 200.0D, 0.0D, 1000.0D, 10.0D);
	...
	public void onAttack(EventReceivePacket event) {
		...
		if (event.getPacket() instanceof C02PacketUseEntity && !(event.getPacket() instanceof S2APacketParticles) && !(event.getPacket() instanceof C0APacketAnimation)) {
			...
			if (packet.getAction() == C02PacketUseEntity.Action.ATTACK && Killaura.target != null && timer.delay(ticks * 5) && mc.thePlayer.onGround) {
				timer.reset();
			}
		}
	}
